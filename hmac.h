...
/// compute HMAC hash of data and key using MD5, SHA1 or SHA256
template <typename HashMethod>
void hmac(const void* data, size_t numDataBytes, const void* key, size_t numKeyBytes, unsigned char * buffer)
{
  // initialize key with zeros
  unsigned char usedKey[HashMethod::BlockSize] = {0};
  ...
  return finalHasher.getHash(buffer);
}
...
/// convenience function for std::string
template <typename HashMethod>
void hmac(const std::string& data, const std::string& key, unsigned char * buffer)
{
  return hmac<HashMethod>(data.c_str(), data.size(), key.c_str(), key.size(), buffer);
}
